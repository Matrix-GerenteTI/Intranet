...
class FlujoEfectivo extends prepareExcel
{
    ...
    const COL_GASTO_GRAL = "AZ";
    const COL_GASTO_GRAL_MERGE ="BA";
  ...
  public static function getDatosARellenar(){
    ...
    public function preparaExcel($fechaInicio, $fechaFin){
		//$mesbuscado = date("m",strtotime($fechaInicio));
		//if($mesbuscado==10)
		//	header("Location: ./estadoresultados.xlsx");
        ...
        //                                         );
        $arrayColumnas = array( array('D','E','F'),array('G','H','I'),array('J','K','L'),array('M','N','O'),array('P','Q','R'),array('S','T','U'),
                                                array('V','W','X'),array('Y','Z','AA'),array('AB','AC','AD'),array('AE','AF','AG'),array('AH','AI','AJ'),array('AK','AL','AM'),
                                                array('AN','AO','AP'), array('AQ','AR','AS') ,array('AT','AU','AV'), array('AW','AX','AY')
                                                        );                                                
        ...
        $this->libro->getActiveSheet()->getStyle("A6:BA".($i-1) )->applyFromArray( $this->bordes );
        $this->libro->getActiveSheet()->getStyle("B6:BA".($i-1) )->applyFromArray( $this->centrarTexto );
        $this->libro->getActiveSheet()->getStyle("A6:A".($i-1) )->applyFromArray( $this->labelBold );

        //HACIENDO LA SUMA DE LOS VALORES INDIVIDUALES
        $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL.'6','GENERAL');
        $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."6:".self::COL_GASTO_GRAL_MERGE."6");
        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."6" )->applyFromArray( $this->labelBold );

        $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL."7","=SUM(D7:AM7)");
        $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."7:".self::COL_GASTO_GRAL_MERGE."7");
        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."7")->getFill()->applyFromArray( $this->colorPadre );
        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."7")->applyFromArray( $this->coloTextPadre );

        $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL."8","=SUM(D8:AM8)");
        $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."8:".self::COL_GASTO_GRAL_MERGE."8");
        $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL."9","=SUM(D9:AM9)");
        $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."9:".self::COL_GASTO_GRAL_MERGE."9");
        $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL."10","=SUM(D10:AM10)");
        $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."10:".self::COL_GASTO_GRAL_MERGE."10");
        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."10")->getFill()->applyFromArray( $this->colorPadre );
        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."10")->applyFromArray( $this->coloTextPadre );
        ...
        foreach ($sucursales  as $index => $sucursal) {
            ...
            $this->libro->getActiveSheet()->getColumnDimension($arrayColumnas[$idxSucursal][2])->setAutoSize(false);
            $this->libro->getActiveSheet()->getColumnDimension($arrayColumnas[$idxSucursal][2])->setWidth("8");
            ...
            $ver = "";
            //comprobando que haya al menos un gasto del tipo CXP para no considerarl el incremento en i
            foreach ($gastosSucursal as $cuenta) {
            ...
            foreach ($gastosSucursal as $idx => $cuenta) {
                ...
                if (  is_numeric( $cuenta['monto']) ){
                    ...
                    //                                                                         var_dump( $cuenta);
                    // echo $sucursal['descripcion']."   valor de i: $i<br><br>";      
                    }
                    ...
                    if ( ($banderaCompras == 1 && (strpos($cuenta['cuenta'] , '601-02')  !== false) )  ){
                        ...
                        // echo "$i  ".$cuenta['monto']."<br>"   ;
                        if( $idx < (sizeof($gastosSucursal)-1) ){
                    ...
                    if ( $cuentaExistente) {
                        $i--;
                          //echo "cuenta no existe".$ver;
                        $cuentaExistente = !$cuentaExistente; //Esto para que solo decremente el valor de i en una sola ocasión
                    }else{
                        if( $idx < (sizeof($gastosSucursal)-1) ){
                ...
                if ( in_array( $cuenta['cuenta'], $cuentasPadre ) ){
                    ...
                    if ( $i >= 7) {
                            // echo $cuenta['cuenta']."      ".$cuenta['monto']."<br><br>";
                        if(  !is_numeric( $cuenta['monto']) )
                            ...
                            if ( $i == 7) { // Es la fila de encabezado de utilidad bruta?
                                ...
                                $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][2]."$i", "=".$arrayColumnas[$idxSucursal][0].($i)."/".$arrayColumnas[$idxSucursal][0].($i+1));
                                ...
                                if( $cuenta['cuenta'] == '601-03-000'){ //ES GASTOS FINANCIEROS
                                    $filaGastoFinanciero  = $i-1; //verificar
                                    $i--;
                                    $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0]."$i", 0);
                                    $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][2]."$i", "=(".$arrayColumnas[$idxSucursal][0].'7'."-(".$arrayColumnas[$idxSucursal][0]. self::GASTO_OP_ROW."+".$arrayColumnas[$idxSucursal][0].$filaGastoFinanciero .") )/".$arrayColumnas[$idxSucursal][0].'8');                                    
                                    $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][2]."$i" )
                                        ->getNumberFormat()->applyFromArray( 
                                            array( 
                                                'code' => PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE
                                    ...
                                    $i++;
                                }elseif( $cuenta['cuenta'] == '601-01-000'){ //ES GASTO DE 
                                    
                                    if ( $i == 10) {
                                        $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0]."$i", "=SUM(".$arrayColumnas[$idxSucursal][0].($i+1).":".$arrayColumnas[$idxSucursal][0].($i+$contadorCuentas).")");
                                    } else {
                                        
                                        $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0]."10", "=SUM(".$arrayColumnas[$idxSucursal][0].($i+1).":".$arrayColumnas[$idxSucursal][0].($i+$contadorCuentas).")");
                                        $this->libro->getActiveSheet()->mergeCells($arrayColumnas[$idxSucursal][0]."10:".$arrayColumnas[$idxSucursal][1]."10");
                                        $i++;
                                ...
                                if ( $statusCorreccio === false ) {
                                    $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0]."$i:".$arrayColumnas[$idxSucursal][2]."$i")->applyFromArray( $this->coloTextPadre );
                                    $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0]."$i:".$arrayColumnas[$idxSucursal][2]."$i")->getFill()->applyFromArray( $this->colorPadre );
                                }
                    }else{
                        $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0].($i+1).':'.$arrayColumnas[$idxSucursal][2].($i+1))->applyFromArray( $this->coloTextPadre );
                        $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0].($i+1).':'.$arrayColumnas[$idxSucursal][2].($i+1))->getFill()->applyFromArray( $this->colorPadre );
                ...
                if ( $cuenta['nombre'] == 'VENTAS EN GENERAL' ) { //Obttienen las ventas
                    ...
                    foreach ($costosVentas as $costoIndividual) {
                        //$costo += $costoIndividual['VENTAS']*0.70;
                    }
                ...
                $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0]."$i")->getNumberFormat()->setFormatCode("$#,##0.00;-$#,##0.00");
                if ( $i < 7) {
                    $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0].($i+1),0 );
                    $this->libro->getActiveSheet()->getStyle($arrayColumnas[$idxSucursal][0].($i+1))->getNumberFormat()->setFormatCode("$#,##0.00;-$#,##0.00");
                    $i +=1;
                }else{
                    $i++;
                }
        ...
        $this->libro->getActiveSheet()->setCellValue("A$i", 'UTILIDAD/PÉRDIDA NETA');
        ...
        for ($i=11; $i <= $ultimaFila ; $i++) { 
            $this->libro->getActiveSheet()->setCellValue(self::COL_GASTO_GRAL."$i","=SUM(D$i:AM$i)");
            $this->libro->getActiveSheet()->mergeCells(self::COL_GASTO_GRAL."$i:".self::COL_GASTO_GRAL_MERGE."$i");
        }
        //Obteniendo el resultado total para aplicar un cambio de color al texto si es perdida o utilidad
        $monto = $this->libro->getActiveSheet()->getCell(self::COL_GASTO_GRAL."$ultimaFila")->getCalculatedValue();
        if ( $monto < 0) {
            $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."$ultimaFila")->applyFromArray($this->setColorText('e60000'));
        }else{
            $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."$ultimaFila")->applyFromArray($this->setColorText('009900'));
        }

        $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."7:".self::COL_GASTO_GRAL.'' .$ultimaFila)->getNumberFormat()->setFormatCode("$#,##0.00;-$#,##0.00");
        ...
        if ( isset( $filaGastoFinanciero ) ) {
                $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."$filaGastoFinanciero")->getFill()->applyFromArray( $this->colorPadre );
                $this->libro->getActiveSheet()->getStyle(self::COL_GASTO_GRAL."$filaGastoFinanciero")->applyFromArray( $this->coloTextPadre );      
        }
             ...
             $movimientosGastos = $estadoFinanciero->getMovimientos($sucursal['id'], $fechaInicio, $fechaFin);
                ...
                if ( $sucursal['id'] == 15 ) { //Es la sucursal de Boulevard
                    ...
                    if (  sizeof($puntoEquilibrioGral) !==  0 ) {
                        ...
                        $this->libro->getActiveSheet()->setCellValue("E".($rowChartGeneral +17),  $puntoEquilibrioGral['monto']);
                        
                        $this->libro->getActiveSheet()->getStyle("E".($rowChartGeneral +17))->getNumberFormat()->setFormatCode("$#,##0.00;-$#,##0.00");
                        ...
                        $this->libro->getActiveSheet()->setCellValue("D".($rowChartGeneral +17), "Monto P.E. : ");
                        $this->libro->getActiveSheet()->setCellValue("E".($rowChartGeneral +17),  "Indefinido");
                    
                    }
                    $this->libro->getActiveSheet()->getStyle("D".($rowChartGeneral +17))->applyFromArray( $this->labelBold);
                ...
                $this->libro->setActiveSheetIndex(0);
                    ...
                    $anterior1 = chr(  ord( $columnasGraficas[$countIndexChart][1] )- 2  );
                    $anterior2 = chr(  ord( $columnasGraficas[$countIndexChart][1] )- 3  );
