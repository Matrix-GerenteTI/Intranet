...
class FlujoEfectivo extends prepareExcel
{
  ...
  public static function getDatosARellenar(){
    ...
    public function preparaExcel($fechaInicio, $fechaFin){
        ...
        $this->libro->getActiveSheet()->setCellValue("$columnasTotal[0]7","=SUM(D7:ZB7)");
        $this->libro->getActiveSheet()->mergeCells("$columnasTotal[0]7:$columnasTotal[1]7");
        ...
        $this->libro->getActiveSheet()->setCellValue("$columnasTotal[0]8","=SUM(D8:ZB8)");
        $this->libro->getActiveSheet()->mergeCells("$columnasTotal[0]8:$columnasTotal[1]8");
        $this->libro->getActiveSheet()->setCellValue("$columnasTotal[0]9","=SUM(D9:ZB9)");
        $this->libro->getActiveSheet()->mergeCells("$columnasTotal[0]9:$columnasTotal[1]9");
        $this->libro->getActiveSheet()->setCellValue("$columnasTotal[0]10","=SUM(D10:ZB10)");
        ...
        foreach ($sucursales  as $index => $sucursal) {
            ...
            $countCuentasOperativas = $estadoFinanciero->getCountCuentasOperativas();
            // if( $sucursal['idprediction'] ){
            //     echo json_encode( $gastosSucursal );
            //     exit();
            // }
            ...
            foreach ($gastosSucursal as $idx => $cuenta) {
                ...
                if (  is_numeric( $cuenta['monto']) ){
                    ...
                    if ( $banderaCompras == 1 && strpos($cuenta['cuenta'] , '601-02')  !== false) {
                        ...
                        $contentComentario = '';
                        echo "<pre>";
                        var_dump($detalleGastos);
                        echo "</pre>";
                        echo $detalleGastos."<br>";
                        echo $cuenta['cuenta']."_".$fechaInicio."_".$fechaFin;
                        ...
                        if ( $i == 7) { // Es la fila de encabezado de utilidad bruta?
                            ...
                            if( $cuenta['cuenta'] == '601-03-000'){ //ES GASTOS FINANCIEROS
                                ...
                                $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0]."$i", 0);
                                    ...
                                    $this->libro->getActiveSheet()->setCellValue($arrayColumnas[$idxSucursal][0]."$i", "=SUM(".$arrayColumnas[$idxSucursal][0].($i+1).":".$arrayColumnas[$idxSucursal][0].($i+$countCuentasOperativas).")");
                                    //if($idx==2);
                        //die('Entra');
        ...
        for ($i=11; $i <= $ultimaFila ; $i++) { 
            $this->libro->getActiveSheet()->setCellValue("$columnasTotal[0]$i","=SUM(D$i:AU$i)");
  ...
  public function listarVentasGastos($fechaInicio, $fechaFin)
  {
      ...
      array('AW','AX','AY'),  array('BA','BB','BC'), array('BE','BF','BG'), array('BI','BJ','BK') , array('BM','BN','BO'), array('BQ','BR','BS'), array('BT','BU','BV'),['BW','BX','BY'],['BZ','CA','CB'],['CC','CD','CE'] );
          ...
          if ( $sucursal['idprediction'] == 0 || is_null($sucursal['idprediction'])  ) {
            ...
            //   Obteniendo los gastos generados por la sucursal
             $movimientosGastos = $estadoFinanciero->getMovimientos($sucursal['id'], $fechaInicio, $fechaFin);
                ...
                // //Establenciendo el ancho de la columna de porcentaje
                // $this->libro->getActiveSheet()->getColumnDimension($columnaVentasGastos[$idxSucursal][2])->setAutoSize(false);
                // $this->libro->getActiveSheet()->getColumnDimension($columnaVentasGastos[$idxSucursal][2])->setWidth(5);
