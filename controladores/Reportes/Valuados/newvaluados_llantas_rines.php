...
*/
    ...
    public function preparaReporte($data)
    {
        ...
        $libroEdicion->setAutoFilter("A8:C8");
        ...
        if($familia == 'LLANTA'){
            ...
            $libroEdicion->setCellValue("M8", 'SAN RAMÓN');
            $libroEdicion->setCellValue("N8", 'MERCALTOS');

            $libroEdicion->setCellValue("O8", 'CENTRO');
            $libroEdicion->setCellValue("P8", 'BOULEVARD');
            
            $libroEdicion->setCellValue("Q8", 'TOTAL');
            ...
            //ESTO SE IRA O CAMBIARA POR RINES

            $libroEdicion->setCellValue("D7", 'TUXTLA');
            $libroEdicion->setCellValue("J7", 'SAN CRISTOBAL');
            $libroEdicion->setCellValue("L7", 'TAPACHULA');

            $libroEdicion->setCellValue("D8", 'LAURELES');
            $libroEdicion->setCellValue("E8", 'QUINTA NTE');
            $libroEdicion->setCellValue("F8", 'ÁMBAR');
            $libroEdicion->setCellValue("G8", 'GÉNESIS');
            $libroEdicion->setCellValue("H8", 'LIBRAMIENTO');
            $libroEdicion->setCellValue("I8", 'PALMERAS');

            $libroEdicion->setCellValue("J8", 'SAN RAMÓN');
            $libroEdicion->setCellValue("K8", 'MERCALTOS');

            $libroEdicion->setCellValue("L8", 'CENTRO');
            $libroEdicion->setCellValue("M8", 'BOULEVARD');

            $libroEdicion->setCellValue("N8", 'TOTAL');

            $libroEdicion->mergeCells('D7:I7');
            $libroEdicion->mergeCells('J7:K7');
            $libroEdicion->mergeCells('L7:M7');
            $libroEdicion->mergeCells('N7:N8');

            $libroEdicion->getStyle("A7:N7")->applyFromArray( $this->labelBold);
            $libroEdicion->getStyle("A7:N7")->applyFromArray( $this->centrarTexto);
            $libroEdicion->getStyle("A7:N7")->applyFromArray( $this->setColorText('ffffff', 11));
            $libroEdicion->getStyle("A7:N7")->getFill()->applyFromArray( $this->setColorFill("073763"));
        }
        ...
        foreach ($listaArticulosDesagrupados as $codigo => $articulo) {
            ...
            if ( $familia  == 'LLANTA' ) {
                    ...
                    $anchoLlanta = substr($descripcionSplitted[0], 0, 3);
                    $altoLlanta = substr($descripcionSplitted[0], 4, 4); // 4 4
                    $rinLlanta = substr($descripcionSplitted[0], 7);
        ...
        $libroEdicion->getStyle("I$i:M$i")->applyFromArray( $this->setColorText("0000cc"));  //cambios
		...
		*/
        ...
        // $valuadoTerminado->save("VALUADOTEST $familia $id.xlsx");
	...
	public function generaReporte( $params )
    {
		...
		// foreach($listReporte as $rw){
            ...
            // exit();
			// $this->enviarReportes($arrfamilias, $arrcorreos, $id, $titulo);         // cambios
		// }
