...
class Articulos extends FirebirdDB
{
    ...
    public function getSubfamiliasAPI( $familia )
    {
        if($familia != 'LLANTA'){
            ...
            $querySubfamilia =  "SELECT     SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)) as subfam,SUBFAMILIA
                                FROM       CFG_ARTICULOS WHERE FAMILIA='LLANTA' 
                                GROUP BY SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)),SUBFAMILIA 
                                HAVING POSITION('/' IN SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)))>0
                                ORDER BY SUBFAMILIA,SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION))";
        }
        return self::fireSelect( $querySubfamilia );
    }
    ...
    {
        $resp = array();
        $querySubfamilia1 =  "SELECT 	CASE CFGA.subfamilia WHEN '' THEN 'OTROS' ELSE CFGA.subfamilia END as SUBFAM
    
                                from 	   cfg_articulos CFGA 

                                where 	CFGA.itemservicio=''
                                AND 		CFGA.familia NOT IN ('LLANTA')
                                GROUP 	by CFGA.subfamilia";
        $s1 = ibase_query($this->conexionFireBird,$querySubfamilia1);
        while($r1 = ibase_fetch_assoc($s1)){
            array_push( $resp ,  ['id' => mb_convert_encoding( $r1['SUBFAM'], "UTF-8") , "name" => mb_convert_encoding( $r1['SUBFAM'], "UTF-8") ] );
        }
        $querySubfamilia2 =  "SELECT     SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)) as SUBFAM
                            FROM       CFG_ARTICULOS WHERE FAMILIA='LLANTA' 
                            GROUP BY SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)),SUBFAMILIA 
                            HAVING POSITION('/' IN SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION)))>0
                            ORDER BY SUBFAMILIA,SUBSTRING(DESCRIPCION FROM 1 FOR POSITION(' ' IN DESCRIPCION))";
        $s2 = ibase_query($this->conexionFireBird,$querySubfamilia2);
        while($r2 = ibase_fetch_assoc($s2)){
            array_push( $resp ,  ['id' => mb_convert_encoding( $r2['SUBFAM'], "UTF-8") , "name" => mb_convert_encoding( $r2['SUBFAM'], "UTF-8") ] );
        }
        return $resp;
    }
        ...
        $queryArticulo = "SELECT FIRST 700  
					...
					for($i=4;$i<=6;$i++){
                        if($this->DameVALORPARAMETRO("cfg_valorcamposgenericos", "valor", "where nombretabla='GENERICO' and nombrecampo='PERMITIR AL USUARIO VER PRECIO PVP".$i." EN CONSULTA RAPIDA DE INVENTARIOS ?' and modulo='REF'", $idusuario) == "S" || $idusuario == 0){
                            ...
                            if($i==4){
                                if($idusuario==8072){
                                //     $queryArticulo.= "(SELECT REPLACE(REPLACE(PA1.PVP3,'$',''),',','') AS PVP3 FROM cfg_preciosxalmacenes PA1 WHERE PA1.fk1mcfg_articulos=CFGA.id AND PA1.fk1mcfg_almacenes=10754) as PVP3,";   
                                // }else{
                                   //$queryArticulo.= "(SELECT CASE WHEN CFGA.familia='LLANTA' THEN REPLACE(REPLACE(PA1.PVP4,'$',''),',','') ELSE '' END AS PVP4 FROM cfg_preciosxalmacenes PA1 WHERE PA1.fk1mcfg_articulos=CFGA.id AND PA1.fk1mcfg_almacenes=10754) as PVP4,";
                                }
                            }
                            if($i==5){
                                if($idusuario==7822){
                                    $queryArticulo.= "(SELECT CASE WHEN CFGA.familia='COLISION' THEN REPLACE(REPLACE(PA1.PVP5,'$',''),',','') ELSE '' END AS PVP5 FROM cfg_preciosxalmacenes PA1 WHERE PA1.fk1mcfg_articulos=CFGA.id AND PA1.fk1mcfg_almacenes=10754) as PVP5,";   
                                }
                            }
                        }
					}			
    ...
    public function buscaArticulosAgrup( $params)
    {
        ...
        $queryArticulo = "SELECT     
                            ...
                            where 	CFGA.itemservicio=''";
        if(trim($almacen)>0){
            $queryArticulo.= "AND 	A2.ID LIKE '%".trim($almacen)."%'";
        }else{
            $queryArticulo.= "AND 	A2.ZONA LIKE '".trim($almacen)."'";
        }
        $queryArticulo.= "  AND     (RAXA.existotal-RAXA.exispedidos-RAXA.exisproceso) > 0 
